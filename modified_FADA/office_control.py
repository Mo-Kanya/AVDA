...
src_dir = "./domain_adaptation_images/amazon/images/"
...
#-------------------training for step 3-------------------
optimizer_all=torch.optim.Adam(list(filter(lambda p:p.requires_grad,encoder.parameters()))+list(classifier.parameters())+list(attention.parameters())+list(discriminator.parameters()), lr=0.0005)
...
for epoch in range(opt['n_epoches_3']):
    ...
    for index in range(n_iters):
        ...
        ground_truth=index_list[index].item()//len(G2)
